"""Oxigraph store construction from PagePile rows."""
...
__all__ = ["PAGEPILE_PREDICATE_BASE", "build_store"]

PAGEPILE_PREDICATE_BASE = "https://pagepile.toolforge.org/ontology/"
...
def _build_store_predicates() -> _StorePredicates:
    return _StorePredicates(
        ...
        namespace=NamedNode(PAGEPILE_PREDICATE_BASE + rdf._field_name("namespace")),
        page_id=NamedNode(PAGEPILE_PREDICATE_BASE + rdf._field_name("page_id")),
        page_title=NamedNode(PAGEPILE_PREDICATE_BASE + rdf._field_name("page_title")),
        wiki_domain=NamedNode(PAGEPILE_PREDICATE_BASE + rdf._field_name("wiki_domain")),
        wiki_dbname=NamedNode(PAGEPILE_PREDICATE_BASE + rdf._field_name("wiki_dbname")),
        wikidata_id=NamedNode(PAGEPILE_PREDICATE_BASE + rdf._field_name("wikidata_id")),
...
def _build_structure_summary(records: Sequence[Mapping[str, Any]]) -> StructureSummary:
    ...
    present_counts = {key: 0 for key in _PAGEPILE_FIELD_ORDER}

    for row in records:
        for key in _PAGEPILE_FIELD_ORDER:
            value = row.get(key)
            if value is None:
                continue
    ...
    for source_key in _PAGEPILE_FIELD_ORDER:
        present_in_rows = present_counts[source_key]
        if present_in_rows <= 0:
            continue
        sparql_type = _PAGEPILE_FIELD_TYPE_BY_KEY[source_key]
        fields.append(
            {
                "source_key": source_key,
                "predicate": _STANDARD_PREDICATE_BY_SOURCE_KEY.get(
                    source_key,
                    PAGEPILE_PREDICATE_BASE + rdf._field_name(source_key),
                ),
...
def _write_record_quads(
    ...
    if not page_url:
        raise PetscanServiceError("PagePile record is missing page_url.")
...
def build_store(
    ...
    try:
        ...
        write_context = _RecordWriteContext(predicates=predicates)
        quad_buffer: List[Any] = []
