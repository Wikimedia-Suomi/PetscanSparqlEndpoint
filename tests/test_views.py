...
class ApiViewTests(SimpleTestCase):
    ...
    @patch("petscan.views.petscan_service.ensure_loaded")
    def test_structure_endpoint_sanitizes_service_errors_with_public_message(self, ensure_loaded):
        ensure_loaded.side_effect = PetscanServiceError(
            "Failed to open Oxigraph store: [Errno 13] Permission denied: '/srv/private/data/123'",
        ...
        self.assertEqual(response.json()["error"], "Local data store is unavailable.")
        self.assertNotIn("/srv/private/data/123", response.content.decode("utf-8"))
    ...
    @patch("petscan.views.petscan_service.execute_query")
    def test_sparql_endpoint_sanitizes_service_errors_with_public_message(self, execute_query):
        execute_query.side_effect = PetscanServiceError(
            "Wikibase enrichment SQL query failed for site fiwiki: (2003, 'db host unreachable')",
            public_message="Failed to enrich linked pages from the replica database.",
        )
        ...
        self.assertNotIn("db host unreachable", response.content.decode("utf-8"))
        self.assertTrue(any("Returning sanitized backend error response" in entry for entry in captured_logs.output))
        self.assertTrue(any("Wikibase enrichment SQL query failed" in entry for entry in captured_logs.output))
