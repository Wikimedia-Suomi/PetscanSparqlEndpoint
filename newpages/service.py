"""Service-layer workflow for new-page ingestion and SPARQL execution."""
...
_NEWPAGES_STORE_ID_OFFSET = 4_000_000_000_000
...
def internal_store_id(
    ...
    user_list_page: Optional[str] = None,
    include_edited_pages: bool = False,
) -> int:
    backend = source.newpages_lookup_backend()
    normalized_wikis = source.normalize_wikis(wiki_domains)
    normalized_timestamp = source.normalize_timestamp(timestamp)
    normalized_user_list_page = source.normalize_user_list_page(user_list_page)
    normalized_include_edited_pages = source.normalize_include_edited_pages(include_edited_pages)
    token = "newpages|backend={}|limit={}|wiki={}|timestamp={}|user_list_page={}|include_edited_pages={}".format(
        backend,
        limit if limit is not None else "",
        ",".join(normalized_wikis),
        normalized_timestamp if normalized_timestamp is not None else "",
        normalized_user_list_page if normalized_user_list_page is not None else "",
        "1" if normalized_include_edited_pages else "",
    )
    digest = hashlib.sha1(token.encode("utf-8")).hexdigest()[:12]
    return _NEWPAGES_STORE_ID_OFFSET + int(digest, 16)
...
def _build_source_params(
    ...
    normalized_wikis = source.normalize_wikis(wiki_domains)
    if normalized_wikis:
        params["wiki"] = normalized_wikis
    normalized_timestamp = source.normalize_timestamp(timestamp)
    if normalized_timestamp is not None:
        params["timestamp"] = [normalized_timestamp]
    normalized_user_list_page = source.normalize_user_list_page(user_list_page)
    if normalized_user_list_page is not None:
        params["user_list_page"] = [normalized_user_list_page]
    if source.normalize_include_edited_pages(include_edited_pages):
        params["include_edited_pages"] = ["1"]
    return params
...
def ensure_loaded(
    ...
    store_id = internal_store_id(
        ...
        include_edited_pages=include_edited_pages,
    )
    ...
    with lock:
        ...
        records, source_url = source.fetch_newpage_records(
            limit=limit,
            wiki_domains=wiki_domains,
            timestamp=timestamp,
            user_list_page=user_list_page,
