...
JSONSTAT_ALLOWED_SOURCE_DOMAINS = tuple(
    _env_list("JSONSTAT_ALLOWED_SOURCE_DOMAINS", default=["stat.fi"])
)
JSONSTAT_TIMEOUT_SECONDS = int(os.getenv("JSONSTAT_TIMEOUT_SECONDS", "30"))
JSONSTAT_MAX_RESPONSE_BYTES = int(os.getenv("JSONSTAT_MAX_RESPONSE_BYTES", str(50 * 1024 * 1024)))
JSONSTAT_MAX_CELLS = int(os.getenv("JSONSTAT_MAX_CELLS", "300000"))
JSONSTAT_CLASSIFICATION_ENRICHMENT_ENABLED = _env_bool(
    "JSONSTAT_CLASSIFICATION_ENRICHMENT_ENABLED",
    default=True,
)
JSONSTAT_CLASSIFICATION_LANGUAGES = tuple(
    _env_list("JSONSTAT_CLASSIFICATION_LANGUAGES", default=["fi"])
)
JSONSTAT_LINKING_SNAPSHOT_PATH = os.getenv(
    "JSONSTAT_LINKING_SNAPSHOT_PATH",
    str(BASE_DIR / "source-data" / "statfi_runtime_links.json"),
).strip()
JSONSTAT_CLASSIFICATION_SNAPSHOT_PATH = os.getenv(
    "JSONSTAT_CLASSIFICATION_SNAPSHOT_PATH",
    JSONSTAT_LINKING_SNAPSHOT_PATH,
).strip()
JSONSTAT_CLASSIFICATION_NETWORK_FALLBACK_ENABLED = _env_bool(
    "JSONSTAT_CLASSIFICATION_NETWORK_FALLBACK_ENABLED",
    default=False,
)
JSONSTAT_CLASSIFICATION_TIMEOUT_SECONDS = int(
    os.getenv("JSONSTAT_CLASSIFICATION_TIMEOUT_SECONDS", "10")
)
JSONSTAT_CLASSIFICATION_MAX_RESPONSE_BYTES = int(
    os.getenv("JSONSTAT_CLASSIFICATION_MAX_RESPONSE_BYTES", str(20 * 1024 * 1024))
)
JSONSTAT_CLASSIFICATION_CACHE_SECONDS = int(
    os.getenv("JSONSTAT_CLASSIFICATION_CACHE_SECONDS", "86400")
)
...
PETSCAN_USER_REGISTRATION_LOOKUP_BACKEND = _env_choice(
    "PETSCAN_USER_REGISTRATION_LOOKUP_BACKEND",
    default="toolforge_sql" if TOOLFORGE_USE_REPLICA else "api",
    choices=("api", "toolforge_sql"),
)
CENTRALAUTH_API_ENDPOINT = os.getenv(
