import {
  ...
  buildPagepileJsonUrl as buildPagepileJsonUrlHelper,
  buildPagepileWizardQuery as buildPagepileWizardQueryHelper,
...
(function () {
  ...
  var app = createApp({
    data: function () {
      return {
        pagepileId: "",
        query: buildPagepileDefaultQueryTextHelper(pagepileRowIdVariableName),
        ...
        meta: {},
        loadedPagepileId: "",
        selectedQueryFieldKeys: defaultPagepileSelectedQueryFields.slice(),
        hasLoadedData: false,
    ...
    computed: {
      ...
      pagepileLimitValue: function () {
        return String(this.pagepileLimit || "").trim();
      },
      effectivePagepileParams: function () {
        var entries = [];
        if (this.pagepileLimitValue) {
          entries.push(["limit", this.pagepileLimitValue]);
        }
        return entries;
      },
      serviceParamPath: function () {
        return buildNamedServiceParamPathHelper("pagepile_id", this.pagepileId, this.effectivePagepileParams, false);
      },
      endpointPreview: function () {
        var base = window.location.origin + pagepileSparqlBasePath;
        if (!this.serviceParamPath) {
          return base + "pagepile_id=<pagepile_id>";
        }
      ...
      jsonResultCount: function () {
        ...
        if (!currentPagepileId || this.loadedPagepileId !== currentPagepileId || !this.meta) {
          return null;
        }
      ...
      querySectionReady: function () {
        var currentPagepileId = String(this.pagepileId || "").trim();
        return Boolean(this.hasLoadedData && currentPagepileId && this.loadedPagepileId === currentPagepileId);
      },
    },
    watch: {
      pagepileId: function () {
    ...
    methods: {
      ...
      structureRequest: async function (pagepileId, refresh) {
        var params = new URLSearchParams();
        params.set("pagepile_id", String(pagepileId || "").trim());
        if (refresh) {
          params.set("refresh", "1");
        }
        this.effectivePagepileParams.forEach(function (entry) {
          params.append(entry[0], entry[1]);
        });

        var response = await fetch(pagepileStructurePath + "?" + params.toString(), {
          method: "GET",
          headers: {
      ...
      sparqlRequest: async function (pagepileId, query, refresh) {
        var servicePath = buildNamedServiceParamPathHelper(
          "pagepile_id",
          pagepileId,
          this.effectivePagepileParams,
          refresh
        );

        var response = await fetch(pagepileSparqlBasePath + servicePath, {
          method: "POST",
          headers: {
      ...
      loadStructure: async function () {
        ...
        this.isBusy = true;
        this.loadExecutionMs = null;
        this.loadStatusMessage = "Loading PagePile data...";
        this.loadStatusLevel = "neutral";
        var loadStartedMs = this.nowMs();

        try {
          var data = await this.structureRequest(this.pagepileId, true);
          ...
          this.meta = data.meta || {};
          this.loadedPagepileId = String(data.pagepile_id || this.pagepileId || "").trim();
      ...
      runQuery: async function () {
        ...
        try {
          ...
          var execution = await this.sparqlRequest(this.pagepileId, this.query, this.refreshBeforeQuery);
          ...
          try {
            var metaData = await this.structureRequest(this.pagepileId, false);
            this.meta = metaData.meta || {};
            this.loadedPagepileId = String(metaData.pagepile_id || this.pagepileId || "").trim();
            if (this.normalizeWizardSelections()) {
              this.updateQueryFromWizardSelections();
            }
          } catch (_metaErr) {
      ...
      buildSparqlServicePath: function (refresh) {
        return buildNamedServiceParamPathHelper("pagepile_id", this.pagepileId, this.effectivePagepileParams, refresh);
      },
      buildPagepileServiceUrl: function (refresh) {
        var servicePath = this.buildSparqlServicePath(refresh);
        return buildPetscanServiceUrlHelper(window.location.origin, pagepileSparqlBasePath, servicePath);
      },
      buildFederatedQueryText: function () {
        return buildFederatedQueryTextHelper(this.buildPagepileServiceUrl(this.refreshBeforeQuery), this.query);
      },
      buildWdqsFederatedQueryViaSophox: function () {
        return buildWdqsFederatedQueryViaSophoxHelper(
          this.buildPagepileServiceUrl(this.refreshBeforeQuery),
          this.query
        );
      },
      buildOpenQueryUrl: function (target) {
        return buildOpenQueryUrlHelper(target, this.query, this.buildPagepileServiceUrl(this.refreshBeforeQuery));
      },
      ...
      buildWizardQuery: function () {
        return buildPagepileWizardQueryHelper(
          this.structureFields,
          this.selectedQueryFieldKeys,
          pagepileRowIdVariableName
        );
      },
