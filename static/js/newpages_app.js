...
(function () {
  ...
  var app = createApp({
    data: function () {
      return {
        query: buildIncubatorDefaultQueryTextHelper(newpagesRowIdVariableName),
        refreshBeforeQuery: false,
        newpagesLimit: "10",
        newpagesWikis: "",
        newpagesTimestamp: "",
        newpagesUserListPage: "",
        newpagesIncludeEditedPages: false,
        isBusy: false,
        statusMessage: "Ready.",
        statusLevel: "neutral",
        ...
        selectedQueryFieldKeys: defaultNewpagesSelectedQueryFields.slice(),
        hasLoadedData: false,
    ...
    computed: {
      ...
      newpagesUserListPageValue: function () {
        return normalizeNewpagesUserListPageHelper(this.newpagesUserListPage);
      },
      showNewpagesIncludeEditedPages: function () {
        return Boolean(this.newpagesUserListPageValue);
      },
      effectiveNewpagesParams: function () {
        var entries = [];
        if (this.newpagesLimitValue) {
          entries.push(["limit", this.newpagesLimitValue]);
        }
        if (this.newpagesWikisValue) {
          entries.push(["wiki", this.newpagesWikisValue]);
        }
        if (this.newpagesTimestampValue) {
          entries.push(["timestamp", this.newpagesTimestampValue]);
        }
        if (this.newpagesUserListPageValue) {
          entries.push(["user_list_page", this.newpagesUserListPageValue]);
        }
        if (this.showNewpagesIncludeEditedPages && this.newpagesIncludeEditedPages) {
          entries.push(["include_edited_pages", "1"]);
        }
        return entries;
      },
      currentFilterSignature: function () {
        return this.effectiveNewpagesParams
          .map(function (entry) {
      ...
      endpointPreview: function () {
        var base = window.location.origin + newpagesSparqlBasePath;
    ...
    watch: {
      newpagesLimit: function () {
        this.hasLoadedData = false;
        this.loadExecutionMs = null;
      },
      newpagesWikis: function () {
        this.hasLoadedData = false;
        this.loadExecutionMs = null;
      },
      newpagesTimestamp: function () {
        this.hasLoadedData = false;
        this.loadExecutionMs = null;
      },
      newpagesUserListPage: function () {
        if (!this.newpagesUserListPageValue) {
          this.newpagesIncludeEditedPages = false;
        }
        this.hasLoadedData = false;
        this.loadExecutionMs = null;
      },
      newpagesIncludeEditedPages: function () {
        this.hasLoadedData = false;
        this.loadExecutionMs = null;
      },
    },
    methods: {
      ...
      resolveUserListPageInput: function () {
      ...
      structureRequest: async function (refresh) {
        ...
        this.effectiveNewpagesParams.forEach(function (entry) {
          params.append(entry[0], entry[1]);
        });

        var requestUrl = newpagesStructurePath;
        if (params.toString()) {
      ...
      buildNewpagesServiceUrl: function (refresh) {
      ...
      loadStructure: async function () {
        this.resolveUserListPageInput();
        this.hasLoadedData = false;
        this.isBusy = true;
        this.loadExecutionMs = null;
        this.loadStatusMessage = "Loading new-page data...";
      ...
      buildOpenQueryUrl: function (target) {
        return buildOpenQueryUrlHelper(target, this.query, this.buildNewpagesServiceUrl(this.refreshBeforeQuery));
      },
